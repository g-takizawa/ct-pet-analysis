...
log_path <- file.path(project_root, "output", "logs", "1all_nonparametric.txt")
table_path <- file.path(project_root, "output", "tables", "1all_nonparametric.csv")
figure_hist_base <- file.path(project_root, "output", "figures", "1all_difference_hist")
figure_bar_base <- file.path(project_root, "output", "figures", "1all_difference_bar")
...
bar_plot <- ggplot(bar_plot_data, aes(x = case, y = diff)) +
  ...
  labs(
    title = "Difference (PET - CT) by Case",
    x = "Case",
    y = "PET - CT"
  ) +
  theme_minimal(base_size = 14) +
...
message("Non-parametric tests complete. Log: ", log_path)
